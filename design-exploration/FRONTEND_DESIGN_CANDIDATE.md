...
> **CANDIDATE ONLY** — 供人工评审，不是最终生产规范，不解锁 F1。
...
先裁决 Library 的信息架构和 Reader 的控件召回方式，再确定 token；不要在同一轮讨论生产组件拆分。评审通过后仍应等待 B3 门禁与单独的 F1 授权。

## 停止声明

本候选文档到 Phase 1 为止。未创建最终 `FRONTEND_DESIGN.md`，未开始生产实现。
