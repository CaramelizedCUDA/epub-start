...
V1、V2、V3、V4 迁移分别在 `BEGIN IMMEDIATE ... COMMIT` 中完成，错误分支执行回滚，避免半完成状态导致永久启动失败。B4 的 V5 也必须使用同一追加迁移规则，并在签发前补齐空库/升级/幂等/后段失败回滚、旧进度补种以及删书/删历史独立语义的变红自证。当前既有绿色套件覆盖空库、V1 升级、重复启动、V2 冲突回滚、V1/V3/V4 回滚断言和图书拥有关系的级联删除；V5 未实现前不得写成已覆盖。未覆盖：真实损坏数据库恢复、磁盘写满/断电等运行态故障。
