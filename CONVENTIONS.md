...
所有模型均按 [AGENTS.md](AGENTS.md) 的任务路由、实现边界、验证和交接检查点执行；验证范围统一使用 [验证策略](docs/VERIFICATION.md)，不因模型执行增加检查，也不把静态结果当成真实运行态。模型与委派候选见 [模型路由](docs/agents/model-routing.md)。Sol/Terra/Luna、其他非 Astra 或型号不明的执行者必须读取 [结构化执行补充](docs/agents/structured-execution.md)，已确认的 GPT-6 Astra 自主选择执行方法但遵守同一契约和验收矩阵。

- 用户指定工作优先；没有指定时按 [TODO.md](TODO.md) 顺序选择首个未完成且无阻塞的实现项，并先阅读其完成标准。按 [docs/README.md](docs/README.md) 的职责表查阅相关资料。
- 实现与本次影响面必跑检查完成后，才勾选实现项；阶段验收另记，未测不能写成已通过，也不阻塞后续独立实现。
- 不在任务范围内重构，不复制现有模块，不跳过阶段。发现冲突、缺少批准依赖或安全/权限风险时，记录阻塞并请求人类决策。委派必须有清晰边界和当前授权，主代理审阅证据；不自动提交、推送或切换 provider。
