...
- [x] 审计 `src-tauri/src` 生产代码：`panic!`/`todo!`/`unimplemented!`/`unreachable!` 0 处；按“包含调用的源码行”统计为 202 行 `.unwrap()`（共 209 次调用），全部在 `#[cfg(test)]` 测试模块；生产 `.expect()` 仅 `lib.rs:93` 事件循环收口；锁 poisoning 均映射为错误（2026-08-14）。
