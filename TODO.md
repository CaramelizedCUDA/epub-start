...
- [ ] **Android 环境阻塞：** 受控 `EpubStart_B2_API35` AVD 已可运行；2026-08-22 已完成新 profile 空白安装、46 份逐份导入、来源/封面预算观察、缺失缓存元数据协调、孤儿/临时文件启动清理、WebView 协议修复后的固定 EPUB 单书首屏读取、复制目标步骤 force-stop 后的清理与重试，以及受控 ENOSPC 的稳定错误回归。单书缓存目录删除后的来源/封面重建也已通过。未完成：后台索引、完整业务数据重建、六轮累计 2 GiB；来源/封面 hard-limit 运行态仍不签发。证据见 `target/android-b2-acceptance-20260822-184117` 与 `target/android-b2-closeout-20260822`，不得用单书读取、部分导入或制品检查替代完整 Android 门禁。
