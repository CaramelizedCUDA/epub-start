...
- [x] 2026-09-01 F2 Reader 首轮导航、分页边界与触控过渡修复（范围性完成，不代表 F2/F3 整体完成）：已测（辅助逻辑/构建）前端 build、Android debug arm64 build、`git diff --check`；已测（Android 环境）荣耀 PPG-AN00 / Android 15 真实 WebView 的底部浮动导航、系统上下安全区、单击隐藏/恢复、实际列宽下约 34.8px 左右正文留白、无可见行侧向串页、按钮翻页、左右滑动翻页及带方向箭头的手机主屏式横向过渡；拖动阶段按 30px/80px 分段位移跟手，松手后页码在收尾过渡期间切换，短滑平滑回弹且不改页码。未测（Android/桌面运行态）：黑鲨本轮 Reader、Android 9 Reader、桌面 Tauri Reader、滚动模式滑动、图片长按、目录/搜索/批注/设置完整交互矩阵、所有 EPUB 样本、横屏/软键盘/深色模式、正式签名和更广 OEM 矩阵；未新增 Rust、SQLite 或 IPC。
