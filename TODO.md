...
- [ ] 完成真实空库、加载、失败/重试、导航返回和不同来源状态的可用性；保留已接入的洞察与书架布局。完成标准：Windows/Android 实际操作上述状态并记录覆盖和未测项，不以静态原型替代。
  ...
  - 2026-09-07：已完成 Windows 隔离运行态与前端收口，证据见 [E1 收尾记录](docs/verification/e1-shelf-closeout-2026-09-07.md)。空库、加载、搜索打开/原版块返回、来源缺失/恢复、解析失败同源重试均已测；Android 黑鲨 profile 制品构建/安装成功，但设备进入安全图案锁屏，Android 运行态保持阻塞，故不勾选。
