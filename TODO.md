...
2026-08-29 F1 第一批已落地生产 `src/`：重建 A+C 应用壳层与左侧导航，接入真实书架列表、阅读时长、继续阅读、离线推荐和独立阅读足迹页；系列/搜索/批注显示明确的后续状态，不使用假数据。已验证：`npm.cmd run build`、`npm.cmd run audit:check`、`git diff --check`；未验证：桌面 Tauri 窗口实际运行、真实 IPC 数据回填、Android WebView/设备运行态、Reader 功能与跨端体验验收。
