...
- [x] 实现选区浮动菜单、纯高亮、批注文字、改色、编辑、删除、列表和点击跳转。
- [x] 保存标准 `cfi_range` 及可独立定位的起止 CFI，重启后恢复高亮；内容只按纯文本渲染。
- [x] 完成标准：跨节点选区、重启恢复、删除级联、长度/颜色校验和 CFI 往返测试通过。
- [x] 2026-08-12 自动验证：`cargo fmt --check`、`cargo check`、`cargo test` 65/65（新增批注服务测试：CFI 字符串数据库精确往返、CFI/选中文本/正文长度上限、颜色规范化、更新保留 `book_id/created_at`、`NOTE_NOT_FOUND` 语义、删除级联）、`npm run build`（tsc + vite）通过；主包 571.20 kB，500 kB 告警保留。跨节点选区、重启恢复、标记点击菜单与主题/排版切换后的标记重绘仍需实机人工验收。
