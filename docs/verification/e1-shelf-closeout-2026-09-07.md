...
## 2026-09-07 新策略下的状态核对

按 `65b5ce6` 引入的验证策略核对：App 原入口返回、BookShelf 的空/加载/失败重试与来源操作接线、libraryStore 的并发状态与上述记录一致；从原验收提交到 `65b5ce6` 的生产目录及依赖锁文件无差异。新增固定 library 回归已有 [流程记录](verification-policy-2026-09-07.md) 中的 4 项通过与红绿自证。

因此关闭 E1 实现项及新策略限定范围的 E1 集成检查，复用本记录 Windows 状态和双机主流程，不重复安装、构建或截图。此结论不把 Android 未测错误/取消/撤权恢复标成通过；这些场景由 E2/E10 承接。上文“保持未勾选”和双机完整补测要求是当时规则下的历史交接，新执行顺序以当前 TODO 与验证策略为准。
