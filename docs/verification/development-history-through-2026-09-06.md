# 开发与验收历史（截至 2026-09-06 文档整理前）

本文件保留原 TODO 的完整记录，包含当时的计划、未完成项和后来被替代的状态，不是当前执行看板。当前顺序与状态以 [TODO](../../TODO.md) 为准。历史数字、设备范围、已测与未测清单不追认为当前版本复测；本机 target 路径不是随仓库分发的证据附件。
