...
- `cargo test`：B2 第五节第二项收口后 146/146 通过，exit 0（当前工作区）。已测：既有 B1/B2 搜索与目录资源覆盖；本次新增系列关系读取、CRUD/审计字段、大小写不敏感名称冲突、单系列替换、卷标修剪及 200/201 边界、重排成员校验、后段更新失败回滚和 `COMMIT` 失败回滚释放；新增 7 个测试及 1 个卷标强化均完成目标变红自证。未测：桌面 legacy shell 与 Android 前端对系列/目录 Command 的实际运行态消费、所有 WebView/OEM 版本、多进程/多连接数据库并发、Android/宿主真实磁盘写满、断电、低存储、长期/2 GiB 导入压力，以及低存储场景下 SQLite page/WAL 实际占用；Android Provider/Activity 仍只有既有设备抽样。
...
- `.unwrap()`：`npm run audit:unwrap` 只扫描 `src-tauri/src`，按“包含调用的源码行”自动统计为 388 行（共 395 次调用），全部位于 `#[cfg(test)]` 测试模块内，无一处出现在生产路径；不再把 `src-tauri/target` 生成代码计入结果。
- `.expect()`：生产代码仅 1 处，`lib.rs:104` 的 `.run(tauri::generate_context!()).expect("error while running tauri application")`。这是 Tauri 事件循环的标准启动收口；`setup` 闭包内的目录/数据库/迁移错误均已用 `map_err` + `?` 转成可诊断错误。风险等级：低。修复任务：可选，B2 保留现状即可。
...
42 个 Command 现在全部为薄适配（参数解析 + 服务调用 + 错误转换）。原 3 个缺口均已修复；B2 新增的 5 个搜索 Command 与系列关系读取 Command 也已接入真实服务：
...
### 0.2.4 Command 注册清单比对 — 42/42 一致，历史文档缺口已修复

Rust 注册（`lib.rs` invoke_handler）42 个 Command，与 [IPC.md](IPC.md)、`src/types/ipc.ts`、`src/lib/tauri.ts` 逐一比对：

- **命名一致**：42 个 Command 名称在 Rust/IPC.md/tauri.ts 三方完全一致，无命名漂移。
...
- **未调用**：19 个系列/标签目录 wrapper（`list_series`…`list_book_tags`，含新增 `list_series_books`）在后端已实现并有测试，但 legacy shell 前端未调用。这符合 B 阶段「前端冻结」策略，不视为缺陷，在 F2 接入。
...
- 已测（辅助逻辑/自动化）：`container.xml`、OPF、NAV、NCX 的正文和 MIME；未知来源错误前缀脱敏；来源不可用/资源缺失、参数、预算、解析、不支持、内部错误的协议状态映射；EPUB 提取、LIKE 与 trigram 查询均把 `cfi` 固定为 `null`，旧索引中的伪 CFI 不再返回。变红证据：新增接口前目标测试编译失败；临时将 NCX MIME 降为通用 XML 后目标断言失败；保留旧伪 CFI 时提取/LIKE/trigram 三条断言分别失败；旧来源前缀和 500 映射分别触发目标失败。当前完整 146/146 通过。
