...
- B2 的初始预算为：来源缓存软/硬上限 256/512 MiB，封面缓存软/硬上限 64/128 MiB；搜索索引另设独立预算，全部可重建数据合计硬上限不超过 1 GiB。达到硬上限且无法安全淘汰时返回稳定资源限制错误，不得删除活动租约、数据库或用户明确保存的内容。
...
这些数字是 B2 的初始工程预算，不表示当前实现已经满足。首次可重复 release 基线建立后只允许收紧；放宽需人工批准并同步 [ROADMAP.md](ROADMAP.md)、[TODO.md](TODO.md) 与 [SECURITY.md](SECURITY.md)。缓存统计/清理若需要成为公开能力，必须先设计 [IPC.md](IPC.md) 契约，禁止预注册 stub。
