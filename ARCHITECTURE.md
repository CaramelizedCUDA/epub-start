...
- `source/` 是来源缓存生命周期的唯一负责人：写入采用临时文件加原子替换，命中更新 `source_cache_entries.last_accessed_at`，淘汰按真实 LRU，并在整个 `SourceLease` 生命周期内保护在用文件。
- `db/` 只持久化缓存元数据；B2 优先复用 `source_cache_entries.cache_size_bytes` 与 `last_accessed_at`。若实现发现 Schema 缺口，只能追加迁移，禁止改写 V2。
- 封面缓存由导入/删除服务与专用清理逻辑共同管理；删除图书、导入失败和数据库无对应记录时不得遗留孤儿文件。搜索索引由搜索服务负责失效、重建与预算，两者均属于可重建数据。
- B2 的初始预算为：来源缓存软/硬上限 256/512 MiB，封面缓存软/硬上限 64/128 MiB；搜索索引 UTF-8 文本字节账面硬上限 256 MiB，全部可重建数据合计硬上限不超过 1 GiB。搜索索引账面预算已在服务层实施；同系列后台索引由任务注册表保证最多一个活动任务，错误状态清理、FTS 重建和状态更新使用单一事务。黑鲨 Android 9 已记录一次 135/135 重建的 SQLite 主库与 rollback journal 占用，但低存储/长期压力和来源/封面缓存淘汰仍待运行态门禁。达到硬上限且无法安全淘汰时返回稳定资源限制错误，不得删除活动租约、数据库或用户明确保存的内容。
- 构建侧初始门禁为 arm64 release APK ≤ 40 MiB、Rust 原生库 ≤ 30 MiB、前端 `dist` ≤ 2 MiB，且相对已提交基线增长超过 10% 必须解释。debug 包只用于诊断；发布候选不得包含原生调试段、测试样本和本机缓存。

来源/封面和发布制品数字是 B2 的初始工程预算，不表示当前实现已经满足；搜索索引 256 MiB 仅是持久化 UTF-8 文本字节账面上限，不替代实际 page/WAL 测量。首次可重复 release 基线建立后只允许收紧；放宽需人工批准并同步 [ROADMAP.md](ROADMAP.md)、[TODO.md](TODO.md) 与 [SECURITY.md](SECURITY.md)。缓存统计/清理若需要成为公开能力，必须先设计 [IPC.md](IPC.md) 契约，禁止预注册 stub。
